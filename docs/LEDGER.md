...
- **[проверка] Опубликованный артефакт.** Скачанный с GitHub portable exe распакован (7zr);
  его собственные `resources/engine/engine.exe` + `resources/ffmpeg/ffmpeg.exe` выполнили реальную
  загрузку (MP3 304 748 байт, событие `done`). Т.е. именно биты, которые скачивает пользователь,
  реально качают видео.
